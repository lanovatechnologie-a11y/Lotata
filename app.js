...
const GameEngine = {
    ...
    checkLotto4Win(bet, res) {
        ...
        const b1 = res.lot1.slice(-2); // Derniers 2 chiffres du 1er lot
        const b2 = res.lot2; // 2e lot (2 chiffres)
        const b3 = res.lot3; // 3e lot (2 chiffres)
        
        let totalGain = 0;
        
        // Option 1: 1er lot & 2e lot (b1 + b2)
        if (APP_STATE.lotto4Options[0] && cleanNum === (b1 + b2)) {
            totalGain += amt * CONFIG.GAMING_RULES.LOTTO4;
        }
        
        // Option 2: 2e lot & 3e lot (b2 + b3)
        if (APP_STATE.lotto4Options[1] && cleanNum === (b2 + b3)) {
            totalGain += amt * CONFIG.GAMING_RULES.LOTTO4;
        }
        
        // Option 3: 1er lot & 3e lot (b1 + b3)
    ...
    checkLotto5Win(bet, res) {
        ...
        const lot1 = res.lot1; // 1er lot (3 chiffres)
        const b2 = res.lot2; // 2e lot (2 chiffres)
        const b3 = res.lot3; // 3e lot (2 chiffres)
        const lastTwoOfLot1 = lot1.slice(-2); // Derniers 2 chiffres du 1er lot
        
        let totalGain = 0;
        
        // Option 1: 1er lot (3 chiffres) & 2e lot (2 chiffres)
        if (APP_STATE.lotto5Options[0] && cleanNum === (lot1 + b2)) {
            totalGain += amt * CONFIG.GAMING_RULES.LOTTO5;
        }
        
        // Option 2: 1er lot (3 chiffres) & 3e lot (2 chiffres)
        if (APP_STATE.lotto5Options[1] && cleanNum === (lot1 + b3)) {
            totalGain += amt * CONFIG.GAMING_RULES.LOTTO5;
        }
        
        // Option 3: 1er lot (3 chiffres) & derniers 2 chiffres du 1er lot
    ...
    checkWinningTicket(bet, res) {
        ...
        switch(bet.game) {
            ...
            case 'lotto3':
                if (cleanNum === lot1) return amt * CONFIG.GAMING_RULES.LOTTO3;
                break;
    ...
    calculateAutoLotto4Gain(bet, res) {
        ...
        if (APP_STATE.lotto4Options[0] && numbers === (b1 + b2)) {
...
// Moteur de vérification des gagnants
const WinnerEngine = {
...
function clearAllData() {
    if (confirm("Èske ou vle efase tout done? Aksyon sa pa ka anile!")) {
        localStorage.clear();
        APP_STATE.ticketsHistory = [];
        APP_STATE.currentCart = [];
        alert("Tout done efase!");
        renderHistory();
        renderReports();
    }
}
